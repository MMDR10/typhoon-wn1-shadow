...
# -*- coding: utf-8 -*-
"""
🌀 850 hPa vs 200 hPa WN1 影子對比
===================================
問題：颱風移動主要由低層（850-500 hPa）引導流控制。
      我哋一直用 200 hPa 徑向風 WN1 相位 — 850 hPa 會唔會更好？
測試：
1. 850 hPa WN1 相位 vs 200 hPa 相位（相關？）
2. 兩層各自嘅前瞻誤差（t → t+12h）
3. 兩層各自 ellipt UQ 後精度
4. 兩層混合（用 ellipt 揀較好嗰層）
"""
import xarray as xr
import numpy as np
import json
...
DATA_ROOT = Path("era5_6typhoon")
ROWS = Path("wn1_forward_validation.json")
OUT = Path("wn1_850_vs_200.json")
...
def main():
    ...
    for r in rows:
        ...
        if mv is None:
            continue
        sh850 = shape_at(fpath, c["lat"], c["lon"], 850)
        sh200 = shape_at(fpath, c["lat"], c["lon"], 200)
        results.append(dict(
            storm=storm, iso=iso, move=mv,
            phi850=sh850["wn1_phi"], phi200=sh200["wn1_phi"],
            ellipt850=sh850["ellipt"], ellipt200=sh200["ellipt"],
            amp850=sh850["wn1_amp"], amp200=sh200["wn1_amp"],
            err850=ang_diff(sh850["wn1_phi"], mv),
    ...
    print('=== 850 hPa vs 200 hPa WN1 對比（前瞻 t→t+12h）===\n')
    stat(results, '850 hPa 全部', 'err850')
    stat(results, '200 hPa 全部', 'err200')
    print()

    # ellipt UQ
    good850 = [x for x in results if x['ellipt850'] <= 0.4]
    good200 = [x for x in results if x['ellipt200'] <= 0.4]
    stat(good850, '850 ellipt<=0.4', 'err850')
    stat(good200, '200 ellipt<=0.4', 'err200')
    print()

    # 相位相關（兩層）
    from scipy import stats as st
    dphi = np.array([ang_diff(x['phi850'], x['phi200']) for x in results])
    print(f'兩層相位差（|phi850 - phi200|）: mean={np.mean(dphi):.1f}°  median={np.median(dphi):.1f}°')
    print(f'  其中 <30°: {np.mean(dphi<30)*100:.0f}%  <60°: {np.mean(dphi<60)*100:.0f}%  >90°: {np.mean(dphi>90)*100:.0f}%')
    print()

    # 每層各自最好嘅樣本
    better850 = sum(1 for x in results if x['err850'] < x['err200'])
    better200 = sum(1 for x in results if x['err200'] < x['err850'])
    print(f'850 較好: {better850}/{len(results)} ({better850/len(results)*100:.0f}%)')
    print(f'200 較好: {better200}/{len(results)} ({better200/len(results)*100:.0f}%)')
    print()

    # 混合策略：兩層 ellipt 都<=0.4 時揀 amp 較大嗰層？或者兩層誤差較細？
    # 理想 oracle：每樣本揀誤差較細嗰層（上限）
    oracle = [min(x['err850'], x['err200']) for x in results]
    print(f'Oracle（每樣本揀最好嗰層）: mean={np.mean(oracle):.1f}  <45={np.mean([x<45 for x in oracle])*100:.0f}%')
    # 混合：ellipt 較細嗰層
    mixed = []
    for x in results:
        if x['ellipt850'] <= x['ellipt200']:
            mixed.append(x['err850'])
        else:
    ...
    # 每颱風
    print('--- 每颱風 850 vs 200 ---')
    for s in sorted({x['storm'] for x in results}):
        sub = [x for x in results if x['storm'] == s]
        e850 = [x['err850'] for x in sub]
        e200 = [x['err200'] for x in sub]
        print(f'  {s}: n={len(sub):>2}  850={np.mean(e850):5.1f}°  200={np.mean(e200):5.1f}°')
